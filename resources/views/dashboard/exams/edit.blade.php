...
@section('breadcrumb-items')
    ...
    <li class="breadcrumb-item active">{{ __('Edit Exam') }}</li>
@endsection
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card mb-6">
            ...
            <div class="card-body pt-4">
                <form method="POST" action="{{ route('dashboard.exams.update', $exam->id) }}"
                    ...
                    <div class="row p-5">
                                ...
                                @if (isset($products) && count($products) > 0)
                                    @foreach ($products as $product)
                                        <option value="{{ $product->id }}"
                                            {{ $product->id == old('product_id', $exam->product_id) ? 'selected' : '' }}>{{ $product->name }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            <select id="is_demo" name="is_demo"
                                ...
                                <option value="1" {{ '1' == old('is_demo', $exam->is_demo) ? 'selected' : '' }}>
                                    {{ __('Demo') }}
                                </option>
                                <option value="0" {{ '0' == old('is_demo', $exam->is_demo) ? 'selected' : '' }}>
                                    {{ __('Full') }}
                                </option>
                            </select>
                            @error('is_demo')
                        ...
                        <div class="mb-4 col-md-12">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                                placeholder="{{ __('Enter exam description') }}" cols="30" rows="10">{{ old('description', $exam->description) }}</textarea>
                            @error('description')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="mt-2">
                        <button type="submit" class="btn btn-primary me-3">{{ __('Edit Exam') }}</button>
                    </div>
                </form>
            </div>
            <!-- /Account -->
